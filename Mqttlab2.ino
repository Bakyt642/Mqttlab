...
void sendValues() {
  if ( mqtt_client.connected() ) {
    if ( lum != -1 )
      mqtt_client.publish(String(topic ).c_str(), String("LUMI/:")+String(lum).c_str()) ;
    if ( tmp != -1 )
      mqtt_client.publish(String(topic).c_str(), String("TEMP/:")+String(tmp).c_str()) ;
    if ( hmdt != -1 )
      mqtt_client.publish(String(topic).c_str(), String("HMDT/:")+String(hmdt).c_str()) ;
  }
}
